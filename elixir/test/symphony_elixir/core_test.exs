defmodule SymphonyElixir.CoreTest do
  ...
  defp assert_due_in_range(due_at_ms, min_remaining_ms, max_remaining_ms) do
    ...
    assert remaining_ms >= max(min_remaining_ms - 1_000, 0)
  ...
  test "agent runner claims pickup shelf work into a working state and stops at a role handoff" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-agent-runner-lifecycle-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      while IFS= read -r line; do
        ...
        case "$count" in
          ...
          3)
            ...
            ;;
          4)
            printf '%s\\n' '{"id":3,"result":{"turn":{"id":"turn-lifecycle-1"}}}'
            printf '%s\\n' '{"method":"turn/completed"}'
            ;;
          5)
            printf '%s\\n' '{"id":3,"result":{"turn":{"id":"turn-lifecycle-2"}}}'
            printf '%s\\n' '{"method":"turn/completed"}'
            ;;
        esac
      done
      ...
      state_updater = fn issue_id, state ->
        send(parent, {:state_update, issue_id, state})
        :ok
      end
        ...
        {:ok,
         [
           %Issue{
             id: "issue-lifecycle",
             identifier: "MT-249",
             title: "Implement then hand off",
             description: "Move to review ready",
             state: "Review Ready"
           }
         ]}
      end
      ...
      assert_receive {:state_update, "issue-lifecycle", "In Progress"}
      ...
      assert hd(turn_texts) =~ "Armada runtime role: implementer"
      assert hd(turn_texts) =~ ~s(moved it from "Agent Ready" to "In Progress")
      assert hd(turn_texts) =~
               "Implementers and fixers must not move directly to Human Review just because work touches UI"
    after
